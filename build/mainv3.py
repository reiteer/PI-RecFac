...
# Função para voltar da tela de reconhecimento facial
def back_to_second_frame():
...
def start_face_recognition():
    ...
    def video_loop():
        ...
        window.after(10, video_loop)  # Chama novamente a função para continuar o loop

    video_loop()

# Função para parar o reconhecimento facial e fechar a webcam
...
# Botão para fechar a tela de reconhecimento facial e voltar
close_button_image = PhotoImage(file=relative_to_assets("button_1.png", ASSETS_PATH_2))
close_button = Button(
    recognition_frame,
    image=close_button_image,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: [stop_face_recognition(), back_to_second_frame()],  # Fecha o reconhecimento e volta
    relief="flat"
)
close_button.place(x=258.0, y=500.0, width=296.0, height=80.0)
